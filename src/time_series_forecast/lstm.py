...
def build_mlp_model(input_shape, forecast_steps):
    model = Sequential([
...
def train_model(X_train, y_train, X_test, y_test, model_name="GDP"):
    input_shape = X_train.shape[1] * X_train.shape[2]
    forecast_steps = y_train.shape[1]

    model = build_mlp_model(input_shape, forecast_steps)
    early_stop = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True)
    model.fit(
        X_train.reshape(X_train.shape[0], -1), y_train.reshape(y_train.shape[0], -1),
        validation_split=0.2, epochs=100, batch_size=16, callbacks=[early_stop], verbose=0
    )
    loss, mae = model.evaluate(X_test.reshape(X_test.shape[0], -1), y_test.reshape(y_test.shape[0], -1), verbose=0)
    print(f"\nTest Loss (MSE) for {model_name}: {loss:.4f}, MAE: {mae:.4f}")
    return model, loss, mae

model_gdp, _, _ = train_model(X_train_gdp, y_train_gdp, X_test_gdp, y_test_gdp, "GDP")
model_so2, _, _ = train_model(X_train_so2, y_train_so2, X_test_so2, y_test_so2, "SO₂")

def predict_and_inverse_transform(model, X_test, target_scaler):
    y_pred = model.predict(X_test.reshape(X_test.shape[0], -1))
    return target_scaler.inverse_transform(y_pred)

y_pred_gdp = predict_and_inverse_transform(model_gdp, X_test_gdp, target_scaler_gdp)
y_pred_so2 = predict_and_inverse_transform(model_so2, X_test_so2, target_scaler_so2)
...
# MAPE
print(f"MAPE GDP: {mean_absolute_percentage_error(y_test_gdp_original, y_pred_gdp):.4f}")
print(f"MAPE SO2: {mean_absolute_percentage_error(y_test_so2_original, y_pred_so2):.4f}")

# Create results dir
os.makedirs("../../results/temporal_analysis/lstm/all_plots", exist_ok=True)
...
def plot_predictions(y_test, y_pred, sample_idx, target_name="GDP"):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(f"../../results/temporal_analysis/lstm/sample_{target_name}_{sample_idx}.png", dpi=300)
    plt.show()
...
def plot_all_together(y_test, y_pred, target_name="GDP"):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/lstm/all_plots/{target_name}.png", dpi=300)
    plt.show()
...
def plot_continuous_predictions(y_test, y_pred, target_name="GDP"):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/lstm/all_plots/continuous_{target_name}.png", dpi=300)
    plt.show()
...
model_gdp.save_weights("../../model/lstm/mlp_gdp.weights.h5")
model_so2.save_weights("../../model/lstm/mlp_so2.weights.h5")
pd.DataFrame(y_pred_gdp).to_csv("../../results/temporal_analysis/lstm/y_pred_gdp.csv", index=False)
pd.DataFrame(y_pred_so2).to_csv("../../results/temporal_analysis/lstm/y_pred_so2.csv", index=False)
