...
# === Save Final Models ===
best_model_gdp.save_weights("../../model/lstm/mlp_gdp_best.weights.h5")
best_model_so2.save_weights("../../model/lstm/mlp_so2_best.weights.h5")
pd.DataFrame(y_pred_gdp).to_csv("../../results/temporal_analysis/lstm/y_pred_gdp_best.csv", index=False)
pd.DataFrame(y_pred_so2).to_csv("../../results/temporal_analysis/lstm/y_pred_so2_best.csv", index=False)

# === Plot as before ===
os.makedirs("../../results/temporal_analysis/lstm/best", exist_ok=True)
...
def plot_predictions(y_test, y_pred, sample_idx, target_name="GDP"):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(f"../../results/temporal_analysis/lstm/best/sample_{target_name}_{sample_idx}.png", dpi=300)
    plt.close()
...
def plot_all_together(y_test, y_pred, target_name="GDP"):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/lstm/best/all_{target_name}.png", dpi=300)
    plt.close()
...
def plot_continuous_predictions(y_test, y_pred, target_name="GDP"):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/lstm/best/continuous_{target_name}.png", dpi=300)
    plt.close()
