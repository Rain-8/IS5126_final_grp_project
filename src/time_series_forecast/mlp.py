...
# Create results dir
os.makedirs("../../results/temporal_analysis/MLP/all_plots", exist_ok=True)
...
def plot_predictions(y_test, y_pred, sample_idx, target_name="GDP"):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(f"../../results/temporal_analysis/MLP/sample_{target_name}_{sample_idx}.png", dpi=300)
    plt.show()
...
def plot_all_together(y_test, y_pred, target_name="GDP"):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/MLP/all_plots/{target_name}.png", dpi=300)
    plt.show()
...
def plot_continuous_predictions(y_test, y_pred, target_name="GDP"):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../../results/temporal_analysis/MLP/all_plots/continuous_{target_name}.png", dpi=300)
    plt.show()
...
model_gdp.save_weights("../../model/MLP/mlp_gdp.weights.h5")
model_so2.save_weights("../../model/MLP/mlp_so2.weights.h5")
pd.DataFrame(y_pred_gdp).to_csv("../../results/temporal_analysis/MLP/y_pred_gdp.csv", index=False)
pd.DataFrame(y_pred_so2).to_csv("../../results/temporal_analysis/MLP/y_pred_so2.csv", index=False)
